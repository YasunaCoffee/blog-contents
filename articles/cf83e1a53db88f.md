...
早速AITuberを作ってみたいと思って読み進めていきましたが、**WSL2環境からWindows上のVOICEBOXへのアクセス方法**が書かれていなかったので自分で調べて設定してみました。AITuberの仮想環境づくりに少しでもお役に立てれば幸いです！思わぬところでつまづいてしまったので、みなさんにもシェアしていきたいと思います。
...
## 詰まったところ
前提として**Windows上にダウンロードしたVOICEVOXアプリケーションにアクセスするには同じWindows環境からじゃないとアクセスできません**。
...
```
Session Status                online                                                                                    Account                       アカウント名 (Plan: Free)                                                                       Version                       3.4.0                                                                                     Region                        Japan (jp)                                                                                Latency                       11ms                                                                                      Web Interface                 http://127.0.0.1:4040                                                                     Forwarding                    https://<作成されたアドレス>.ngrok-free.app -> http://localhost:50021                                                                                                                         Connections                   ttl     opn     rt1     rt5     p50     p90                                                                             0       0       0.00    0.00    0.00    0.00    
```
