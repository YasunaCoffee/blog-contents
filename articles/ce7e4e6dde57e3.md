...
AIキャラクターを作ることが好きで、オリジナルアプリケーションでもAIキャラクターに関するものを作ってみたいなとずっと思っていのたで
今回はバレンタインをテーマにAIキャラクターと話せたら楽しいんじゃないか！？ということで開発を始めました。
...
ユーザーは名前とプロフィールを入力することでキャラクターと同じ学校に通う生徒になります。
そして、一番のポイントはユーザープロフィールをもとにAIエージェントが性格分析をして
性格に合わせてキャラクターがあなたとの日記を想像してくれます。
...
そして当日だけではなく、その前日にあなたと過ごした想像上の学校生活の日記を読むことができます。

それにより「いきなり出会う」という訳ではなく、ある程度あなたとの関係性を構築できているということを担保にしながら
バレンタイン当日の会話に対しての感情をより楽しむことができるようになっています。

ユーザーが登場する日記を書いたどの子があなたの本命の子なのか、友達なのか、実は全く脈がないのか、、、
感情分析エージェントが会話をもとに感情を分析してリアルタイムに返答へと反映します。

キャラクターの感情による返答はユーザーであるあなたに大きく委ねられているため、
学生時代のバレンタインデーノリをより楽しむことができ、出会い的制約を解決します。
...
エンターテイメントのロールプレイには社会的性別は関係ないということで解決します。

## システム構成
本アプリのシステム構成を紹介します。
こちらはリポジトリの情報をもとにAIが作成したものです。助かります。
...
# AIエージェントの実装
日記生成と感情分析に関してAIエージェントの実装をしているので、その実装を詳しく紹介させてください。
